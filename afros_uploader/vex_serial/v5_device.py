...
from afros_uploader.util import ui
from afros_uploader.util import *
from semantic_version import Spec
from . import bytes_to_str, decode_bytes_to_str
from .base_port import BasePort
from . import list_all_comports
...
class V5Device(VEXDevice, SystemDevice):
    vid_map = {'user': 1, 'system': 15, 'rms': 16, 'afros': 24, 'mw': 32}  # type: Dict[str, int]
    ...
    def generate_ini_file(self, remote_name: str = None, slot: int = 0, ini: ConfigParser = None, **kwargs):
        ...
        project_ini['program'] = {
            ...
            'description': kwargs.get('description', 'Created with AFROS'),
    ...
    def ensure_library_space(self, name: Optional[str] = None, vid: int_str = None,
        ...
        if len(used_libraries) > 3:
            ...
            if confirm(f'There are too many files on the V5. AFROS can remove the following suggested old files: '
                       f'{evicted_file_list}',
    ...
    def upload_library(self, file: typing.BinaryIO, remote_name: str = None, file_len: int = -1, vid: int_str = 'afros',
